...
const readline = require('readline');                                   // 新增
  ...
  rl.on('line', (line) => {
    ...
    switch (cmd) {
      ...
      case 'quit':
        ...
        return;
      default:
        if (cmd) console.log(`  未知命令: ${cmd} ，输入 help 查看可用命令`);
        break;
    }
    rl.prompt();
  }).on('close', () => {
    // 如果 readline 被关闭（例如 Ctrl+C），也关闭服务器
    console.log('\n  退出交互');
